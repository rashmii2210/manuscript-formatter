...
def upload_file():
    ...
    try:
        raw_text, extracted_images = parse_document(filepath, ext, session_id, app.config['UPLOAD_FOLDER'])        
        structured_data = analyze_structure(raw_text)
        structured_data["images"] = extracted_images
...
def build_latex(rules, structured_data):
    ...
    # Force graphicx package just in case Gemini forgot it
